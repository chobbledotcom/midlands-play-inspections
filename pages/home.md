---
name: Home
meta_title: Inflatable Inspections & PIPA Testing in the Midlands
meta_description: RPII inflatable inspections and PIPA testing across the Midlands. Annual BS EN 14960 safety inspections for bouncy castles, slides, obstacle courses and soft play, booked around your hire season.
permalink: "/"
schema_type: organization
eleventyNavigation:
  key: Home
  order: 1
blocks:
  - type: hero
    class: gradient
    badge: RPII inflatable inspections
    content: |
      # Your inflatables, tested properly, by someone who hires them out too

      Annual PIPA and RPII inspections to BS EN 14960 for bouncy castles, slides,
      obstacle courses, soft play and disco domes. Mobile testing across the
      Midlands, booked around your season rather than against it.
    buttons:
      - text: Book an inspection
        href: /contact/
        variant: primary
        size: lg
      - text: See what it costs
        href: /pricing/
        variant: secondary
        size: lg

  - type: features
    intro_content: |
      ## Why operators book with us

      We run a hire fleet of our own. We know what a failed unit does to a
      Saturday, and we know what a council or a school actually asks to see.
    items:
      - icon: "hugeicons:certificate-01"
        name: RPII registered
        description: Inspections carried out by a registered inflatable inspector, working to BS EN 14960-1:2019.
      - icon: "hugeicons:tag-01"
        name: PIPA tags issued
        description: Qualifying units get a PIPA tag and a report on the national database, so hirers can check it themselves.
      - icon: "hugeicons:truck-delivery"
        name: We come to you
        description: We test at your unit, yard or storage, so you don't have to haul a fleet of castles across the country.
      - icon: "hugeicons:calendar-03"
        name: Off-season friendly
        description: Winter and midweek slots so your equipment is tested when it isn't earning.
      - icon: "hugeicons:file-validation"
        name: Reports that stand up
        description: A clear PDF report you can forward straight to a venue, a school or your insurer.
      - icon: "hugeicons:wrench-01"
        name: Faults explained
        description: If something fails, you get told what it is, why it matters and what will put it right.

  - type: stats
    items:
      - value: "140+"
        label: Check points per inflatable
      - value: "14960"
        label: The standard we test to
      - value: "12"
        label: Months between inspections
      - value: "2011"
        label: Hiring inflatables since

  - type: split-callout
    subtitle: The short answer
    content: |
      ## Does my bouncy castle legally need testing?

      If you hire inflatables out for money, they have to be inspected. Health
      and safety law requires equipment offered to the public for commercial
      hire to have in-service safety inspections, and HSE guidance expects
      amusement devices to be checked **annually by a competent person**.

      PIPA is one of two inspection schemes named in HSE's guidance, HSG175.
      A current PIPA tag is the quickest way to show a venue, a school or an
      insurer that the unit in front of them has been through that check.

      Domestic owners aren't covered by the same duty, but if you lend or hire
      your unit to anyone else, you are.
    button:
      text: Read the full answer
      href: /faqs/
      variant: secondary
    figure_icon: "hugeicons:shield-01"
    figure_name: Annual inspection
    figure_subtitle: BS EN 14960-1:2019 · HSG175 · PUWER
    figure_variant: primary

  - type: split-full
    variant: dark-left
    left_content: |
      ## What we inspect

      Bouncy castles and bounce-and-slide units. Inflatable slides, obstacle
      courses and assault courses. Disco domes, soft play and ball pools.
      Bungee runs, gladiator duels and other non-ride-on games. Sealed
      inflatables and units outside the PIPA scope get an EN 14960 or
      bespoke inspection instead.
    left_button:
      text: Full equipment list
      href: /what-we-inspect/
      variant: primary
    right_content: |
      ## Where we cover

      Based in Warwickshire, covering Coventry, Birmingham, Solihull, Rugby,
      Leamington, Warwick, Stratford, Redditch, Worcester, Leicester,
      Northampton, Wolverhampton, Telford, Derby, Nottingham and the towns
      in between. Further afield by arrangement, because we travel for fleet days.
    right_button:
      text: Check your area
      href: /areas-we-cover/
      variant: secondary

  - type: features
    center: true
    intro_content: |
      ## How a booking works

      There are six steps to it and none of them involve chasing paperwork.
    items:
      - icon: "hugeicons:message-01"
        name: 1. Send your list
        description: Makes, rough sizes and quantities are enough for us to price it properly.
      - icon: "hugeicons:money-01"
        name: 2. Get a fixed price
        description: A firm figure in writing, with travel included, before anything is booked.
      - icon: "hugeicons:calendar-check-in-01"
        name: 3. Pick a date
        description: We come to you, for one unit or a full fleet day, on weekdays or winter weekends.
      - icon: "hugeicons:search-visual"
        name: 4. We inspect
        description: We check anchorage, seams, pressure, entrapments, fall heights, the blower and the electrics.
      - icon: "hugeicons:tag-01"
        name: 5. Tags go on
        description: They're fitted before we leave, so you can take bookings the same afternoon.
      - icon: "hugeicons:mail-send-01"
        name: 6. The report lands
        description: A PDF within a few days, with any faults written out in plain English.

  - type: cta
    content: |
      ## Ready to get your fleet tested?

      Send a list of your units and we'll come back with a price and the next
      free date. Fleet days across the Midlands work out cheapest per unit.
    button:
      text: Get a price
      href: /contact/
      variant: secondary
      size: lg
---
